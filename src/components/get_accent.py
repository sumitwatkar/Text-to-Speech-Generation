...
def get_accent(user_input):
    try:
        ...
        logger.error(f"An error occurred in get_accent function: {e}")
...
def get_accent_message():
    try:
        accent = ['American', 'Australian', 'British', 'Canadian', 'Indian', 'Spanish']
        return accent
    
    except Exception as e:
        logger.error(f"An error occurred in get_accent_message function: {e}")
        raise CustomException(e, sys)